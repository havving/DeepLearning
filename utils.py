...
def read_data_path(x, y):
    inputs_path = [os.path.join(x, file) for file in os.listdir(x) if file.endswith('.jpg')]
    labels_path = [os.path.join(y, file) for file in os.listdir(y) if file.endswith('.png')]

    print('inputs_path_len : {}'.format(len(inputs_path)))
    print('labels_path_len : {}'.format(len(labels_path)))
...
def read_annotation(path, resize):
    ...
    for i in range(len(path)):
        ...
        label = np.array(Image.fromarray(label).resize(resize))
